...
export function Patients() {
  ...
  const canAddPatients = profile?.role !== "patient" && profile?.role !== "client";
  ...
  return <Stack spacing={3}>
    <Box sx={{ display: "flex", alignItems: { xs: "flex-start", sm: "center" }, justifyContent: "space-between", flexDirection: { xs: "column", sm: "row" }, gap: 1.5 }}>
      ...
      <Stack direction={{ xs: "column", sm: "row" }} spacing={1} alignItems={{ sm: "center" }}><Chip icon={<ShieldRounded />} label={scopeClinic ? "Klinika doirasida" : "Super admin ko‘rinishi"} sx={{ bgcolor: "#EFF4FF", color: "#175CD3" }} />{canAddPatients && <Button variant="contained" startIcon={<AddRounded />} onClick={() => setShowAddPatient(true)}>Bemor qo‘shish</Button>}</Stack>
    </Box>
    ...
    {canAddPatients && showAddPatient && <NewPatientDialog profileRole={profile?.role ?? "medical_worker"} fixedClinicId={scopeClinic} facilities={facilities} onClose={() => setShowAddPatient(false)} onSave={addPatient} />}
